...
logging.basicConfig(filename='slave_two.log', filemode='w', format='%(message)s', level=logging.INFO)

# Slave node
slave_private_key = '94ba95a4520107a8a9abc864db900b5c00660c48bbb0333edaa5ab081f52e2ed'
slave_public_key = IrohaCrypto.derive_public_key(slave_private_key)
slave_name = 'slave2'
domain_id = 'choice'
slave_account_id = slave_name + '@' + domain_id
iroha_slave = Iroha('slave2@choice')
...
def model(beta_car, beta_cost, beta_tt):
    ...
    """
    models = []
    with open('data_two.csv') as file:
