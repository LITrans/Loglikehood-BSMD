...
logging.basicConfig(filename='slave_four.log', filemode='w', format='%(message)s', level=logging.INFO)

# Slave node
slave_private_key = 'afa0c9cd046cbf7cb8998761c28a3dbfd8537de02d078657dcce25614a34b2d9'
slave_public_key = IrohaCrypto.derive_public_key(slave_private_key)
slave_name = 'slave4'
domain_id = 'choice'
slave_account_id = slave_name + '@' + domain_id
iroha_slave = Iroha('slave4@choice')
...
def model(beta_car, beta_cost, beta_tt):
    ...
    """
    models = []
    with open('data_four.csv') as file:
